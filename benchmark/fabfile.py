...
def localShard(ctx):
    """Run benchmarks on localhost"""
    bench_params = {
        "faults": 0.0,      # actual crash ratio in the ordering shard
        "nodes": 10,        # the number of nodes (shard size) in the ordering shard
        "rate": 2_000,      # transaction sending rate per execution shard
        "tx_size": 512,     # transaction size
        "cross_shard_ratio": 0.2,   # ratio of cross-shard transactions
        "duration": 60,     # testing time/s
        "liveness_threshold": 0.41, # liveness threshold per execution shard
        "shard_faults": 0.0,    # actual crash ratio in an execution shard
        "shard_num": 3,     # the number of execution shards
        "shard_sizes": 4,   # the number of nodes (shard size) in per execution shard
    }
    node_params = {
        "consensus": {
            "timeout_delay": 3_000, # timeout delay  in the ordering shard
            "sync_retry_delay": 10_000,
            "cblock_batch_size": 5_000, # maximum certificate blocks can be ordered per consensus round
        },
        "mempool": {
            "gc_depth": 50,
            "sync_retry_delay": 5_000,
    ...
    executor_params = {
        ...
        "mempool": {
            ...
            "certify_sync_retry_delay": 5_000,
            "certify_sync_retry_nodes": 3,
            "certify_batch_size": 500_000,      # batch size of an execution shard
